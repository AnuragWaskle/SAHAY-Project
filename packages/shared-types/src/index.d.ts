export type UserRole = 'citizen_guest' | 'citizen' | 'verified_citizen' | 'civic_leader' | 'ngo' | 'company_csr' | 'society_rwa' | 'municipal_officer' | 'elected_representative' | 'political_party' | 'police' | 'journalist' | 'sub_admin' | 'super_admin';
export type BadgeType = 'blue_tick' | 'green_tick' | 'gold_star' | 'grey_check' | 'press_badge' | 'none';
export type PrivacyLevel = 'public' | 'semi_private' | 'private';
export type CivicLevel = 1 | 2 | 3 | 4 | 5;
export declare const CIVIC_LEVELS: Record<CivicLevel, {
    name: string;
    emoji: string;
    minScore: number;
}>;
...
export type ReportCategory = 'waterlogging' | 'pothole' | 'garbage' | 'streetlight' | 'water_supply' | 'sewage' | 'road_damage' | 'encroachment' | 'tree_hazard' | 'air_pollution' | 'noise_pollution' | 'park_damage' | 'stray_animals' | 'safety' | 'other';
export type Severity = 'low' | 'medium' | 'high' | 'critical';
export type ReportStatus = 'submitted' | 'ai_processed' | 'clustered' | 'standalone' | 'resolved' | 'rejected';
...
export type DemandStage = 'proposed' | 'community_supported' | 'submitted' | 'accepted' | 'work_planned' | 'in_progress' | 'completed' | 'citizen_verification' | 'resolved' | 'reopened';
...
export type NotificationType = 'incident_near_you' | 'demand_stage_change' | 'verification_request' | 'mission_update' | 'reward_earned' | 'sos_alert' | 'report_clustered' | 'verification_result';
